...
public class police_AllDriverinfo extends RecyclerView.Adapter<police_AllDriverinfo.ViewHolder>{
    Context context;
    List<driver> allDriverList;

    public police_AllDriverinfo(Context context , List<driver> driverList) {
    ...
    @NonNull
    @Override
    public police_AllDriverinfo.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.all_item_layout, parent, false);
        return new police_AllDriverinfo.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull police_AllDriverinfo.ViewHolder holder, int position) {
